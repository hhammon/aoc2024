...
main:
	...
	__main_loop_find_dos_and_donts:
	add ecx, 8
	cmp ecx, esi
	jg __main_dos_and_donts_found
	sub ecx, 8

	# bool dont = false in r8
	# if (dont)
	test r8, r8
	jz __main_loop_find_dos_and_donts_do

	# dont = buf[i..i+4] != "do()"
	cmp dword ptr [rdi + rcx], 0x29286f64 # "do()"
	setne r8b

	# I'm lazy... I'll just zero out the memory in `don't` regions.
	mov byte ptr [rdi + rcx], 0
	inc ecx

	jmp __main_loop_find_dos_and_donts

	__main_loop_find_dos_and_donts_do:
	# dont = buf[i..i+7] == "don't()"
	cmp dword ptr [rdi + rcx], 0x276e6f64 # "don'"
	sete r8b
	add ecx, 4
	cmp word ptr [rdi + rcx], 0x2874 # "t("
	sete r9b
	and r8, r9
	add ecx, 2
