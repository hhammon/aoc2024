...
can_equate:
	...
	# long ops_mask = (1 << (2 * (len - 2))) - 1; // [rsp + 24]
	mov ecx, esi
	sub ecx, 2
	mov eax, 1
	add ecx, ecx
...
check_equation:
	...
	# if, in base 4, any of ops digits is 3, return false
	...
	and r9, rax
	test r8, r9
	jz __check_equation_valid_ops

	xor eax, eax
	ret

	__check_equation_valid_ops:
	...
	# if (ops & 2) concatenate rhs and values[i]
	test rsi, 2
	jz __check_equation_no_concat

	# long power_of_ten = 1;
	mov r10, 1

	# move values[i] to r11
	mov r11, qword ptr [rdi + 8 * rcx]

	# while (power_of_ten <= values[i]) {rhs *= 10; power_of_ten *= 10;}
	__check_equation_loop_mul_by_ten:
	cmp r10, r11
	jg __check_equation_loop_mul_by_ten_done
	...
	jmp __check_equation_loop_mul_by_ten

	__check_equation_loop_mul_by_ten_done:
