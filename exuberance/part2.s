...
main:
	...
	jnz __main_loop_continue

	# Sort update to conform to rules
	# sort_update(pages.ptr + pages_idx, lengths.ptr[lengths_idx], &rule_set)
	mov rdi, qword ptr [rsp + 48]
	mov esi, dword ptr [rdi + 4 * rdx]
...
# void sort_update(int *update, int update_len, RuleSet *rule_set);
sort_update:
	# int i = 0;
	xor r8d, r8d

	__sort_update_loop_outer:
	cmp r8d, esi
	jge __sort_update_loop_outer_done

	# int j = i + 1;
	mov r9d, r8d
	inc r9d

	__sort_update_loop_inner:
	cmp r9d, esi
	jge __sort_update_loop_inner_done

	# if (is_rule(rule_set, update[j], update[i])) swap(&update[i], &update[j]);
	push rdi
	push rsi
	push rdx
	push r8
	...
	inc r9
	test eax, eax
	jz __sort_update_loop_inner

	dec r9

	mov r10d, dword ptr [rdi + 4 * r8]
	xchg r10d, dword ptr [rdi + 4 * r9]
	mov dword ptr [rdi + 4 * r8], r10d

	jmp __sort_update_loop_outer # update[i] Needs to be checked again

	__sort_update_loop_inner_done:

	inc r8
	jmp __sort_update_loop_outer

	__sort_update_loop_outer_done:
	ret
