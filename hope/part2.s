...
find_antinodes_for_antennas:
	...
	# Coordinates antinode1 = antenna1; // [rsp + 16]
	mov qword ptr [rsp + 16], rsi

	# Coordinates antinode2 = antenna2; // [rsp + 24]
	mov qword ptr [rsp + 24], rdx
	
	# Coordinates diff = antenna2 - antenna1; // [rsp + 8]
	mov eax, edx
	sub eax, esi
	...
	# while(grid_set(antinodes, antinode1.x, antinode1.y, '#')) antinode1 -= diff;
	__find_antinodes_for_antennas_loop_1:
	mov rdi, qword ptr [rsp]
	...
	test al, al
	jz __find_antinodes_for_antennas_loop_1_done
	
	mov eax, dword ptr [rsp + 8]
	sub dword ptr [rsp + 16], eax

	mov eax, dword ptr [rsp + 12]
	sub dword ptr [rsp + 20], eax

	jmp __find_antinodes_for_antennas_loop_1

	__find_antinodes_for_antennas_loop_1_done:

	# while(grid_set(antinodes, antinode2.x, antinode2.y, '#')) antinode2 += diff;
	__find_antinodes_for_antennas_loop_2:
	mov rdi, qword ptr [rsp]
	...
	test al, al
	jz __find_antinodes_for_antennas_loop_2_done
	...
	jmp __find_antinodes_for_antennas_loop_2

	__find_antinodes_for_antennas_loop_2_done:
