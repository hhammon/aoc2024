...
# void int_array_qsort(IntArray *arr);
int_array_qsort:
	# quicksort_recursive(arr->ptr, 0, arr->len - 1)
...
# void quicksort_recursive(int *ptr, int low, int high);
quicksort_recursive:

	# if (low >= high) return;
	cmp rsi, rdx
	jge __quicksort_recursive_done

	push rdi
	push rsi
	push rdx

	# (pl, ph) = partition(ptr, low, high)
	call partition
	mov rcx, rdx
	...
	# quicksort_recursive(ptr, low, pl - 1);
	push rdi
	push rdx
	push rcx
	...
	# quicksort_recursive(ptr, ph + 1, high)
	mov rsi, rcx
	inc rsi
	call quicksort_recursive

	__quicksort_recursive_done:
	ret

# (unsigned long, unsigned long) partition(int *ptr, int low, int high);
# This uses a three-way partition.
# Returns (low, high) in (rax, rdx)
partition:
	# int pivot = ptr[(low + high) / 2];
	mov r8, rsi
	add r8, rdx
	shr r8
	mov r8d, dword ptr [rdi + 4 * r8]
	...
	# if (item < pivot)
	cmp eax, r8d
	jge __partition_loop_not_less

	# (ptr[il], ptr[im]) = (ptr[im], ptr[il])
	mov ecx, dword ptr [rdi + 4 * r9]
	mov dword ptr [rdi + 4 * r9], eax
	mov dword ptr [rdi + 4 * r10], ecx

	# il++; im++;
	inc r9
	inc r10

	jmp __partition_loop

	__partition_loop_not_less:
	# if (item > pivot)
	je __partition_loop_equal

	# (ptr[im], ptr[ih]) = (ptr[ih], ptr[im])
	mov ecx, dword ptr [rdi + 4 * r11]
	mov dword ptr [rdi + 4 * r11], eax
	mov dword ptr [rdi + 4 * r10], ecx
